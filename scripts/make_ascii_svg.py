...
HANDLE = "ritessshhh"        # terminal prompt user
NAME = "Ritesh Chavan"       # shown after `whoami`
...
CONTRAST = 1.08
BRIGHTNESS = 1.0
GAMMA = 1.08          # >1 brightens mids -> face lands in sparser chars
SHARPEN = False
...
FRAME = T["frame"]
TITLE_TEXT = T["muted"]
INK = T["ink"]       # the single ascii color
CURSOR = T["ink"]
...
# --- denoise: clear faint stray marks ABOVE the head -----------------------
# rembg leaves faint halo specks over the hair that sample to sparse chars.
# find the first row carrying real head/hair density and blank everything above.
DENSE = set(RAMP[7:])          # '*', 'c', 's', '#', '%', '@'
# noise specks are all LIGHT chars (0 dense); real hair has dark/dense chars, so
# a low threshold clears the specks while keeping the faint hairline (head sits
# higher). raise this if stray marks return above the head.
HEAD_MIN = 2                   # dense chars needed to call a row "the head"
head_top = next((y for y, line in enumerate(rows_txt)
                 if sum(c in DENSE for c in line) >= HEAD_MIN), 0)
if 0 < head_top < ROWS // 2:
    for y in range(head_top):
        rows_txt[y] = " " * COLS

# push the portrait up: drop the leading blank rows (keeping a little headroom)
# and backfill blanks at the bottom, so the head rides higher in the frame.
HEADROOM = 1                   # blank rows to keep above the head
lead = next((y for y, line in enumerate(rows_txt) if line.strip()), 0)
shift = max(0, lead - HEADROOM)
if shift:
    rows_txt = rows_txt[shift:] + [" " * COLS] * shift
...
# status bar with a steady blinking cursor after `whoami <name>`
status_line_y = TITLEBAR_H + ART_H + PAD * 0.35
status_y = status_line_y + 19
prefix = f"{HANDLE}@github:~$ whoami "
CW = 7.35  # monospace advance at font-size 13
cursor_x = PAD + (len(prefix) + len(NAME) + 1) * CW
parts.append(f'<line x1="0" y1="{status_line_y:.1f}" x2="{CANVAS_W}" y2="{status_line_y:.1f}" stroke="{FRAME}"/>')
parts.append(f'<text x="{PAD}" y="{status_y:.1f}" fill="{TITLE_TEXT}" font-size="13">'
             f'{html.escape(prefix)}<tspan fill="{INK}">{html.escape(NAME)}</tspan></text>')
